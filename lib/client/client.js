...
module.exports = function(responderId, config, send) {
  ...
  ss.registerApi('resetModelParams', function(){
    var req = parseRequest(arguments);
    if (!callbacks[req.requestname]){
      throw new Error("Request to reset parameters for an unknown model.");
    }
    else {
      console.log("Resetting model params " + req.requestName + " with the server");
      var request = {
        m: "PARAMS",
        n: req.modelName,
        p: req.params
